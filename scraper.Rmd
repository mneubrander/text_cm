...
```{r}
not_found = df_arx[is.na(df_arx$matched_title),]
rn_nf = as.numeric(row.names(not_found))
not_found$ab = NA
```

```{r}
for(j in 1:nrow(not_found)){
  id = not_found$id[j]
  abstract <- df[df$id == id, "abstract"]
  not_found$ab[j] = abstract
}
```

```{r}
write.csv(not_found, "not_found.csv", row.names = TRUE)
```
...
```{r}
df_results_2$submitted_date <- as.POSIXct(df_results$submitted, format="%Y-%m-%d %H:%M:%S")
df_results_2$submitted_comp = NA
```


```{r}
target_date <- as.POSIXct("2017-11-27 00:00:00")
```


```{r}
df_results_2$submitted_comp <- ifelse(
  is.na(df_results_2$submitted_date), 
  "after",
  if_else(
  df_results_2$submitted_date < target_date, 
  "before", 
  ifelse(df_results_2$submitted_date == target_date, 
         "on", 
         "after")
  )
)
